...
const Price = () => {
  ...
  return (
   ...
   <input className=' w-2/4 h-[50px] ml-[5px] rounded-[10px] border border-black shadow-2xl' placeholder='count...' type="number" onChange={(e)=>setQuantity(e.target.value)}/>
      ...
      <div className="text-center text-[30px]  "><button className='border  hover:bg-violet-500 sm1:p-[10px] lg:p-[15px] 2xl:w-[200px] shadow-lg rounded rounded-[9px] bg-violet-600 font-semibold font-poppins text-white'><Link to={`/placeOrder/${name}/${cost}/${quantity}/${totalPrice}`}>place oder </Link></button></div>
      </div>
